...
for (const [index, row] of rows.entries()) {
  ...
  ids.add(row.id);
  if (!["fulfilled", "refunded"].includes(row.outcome)) {
    errors.push(`row ${row.id} has invalid outcome ${row.outcome}`);
  }
  for (const field of ["buyerPaidWarrantyTx", "warrantyPaidTargetTx", "warrantyDeliveredTx"]) {
...
const refunded = rows.filter((row) => row.outcome === "refunded").length;
